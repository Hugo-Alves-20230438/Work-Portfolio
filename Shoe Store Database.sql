DROP DATABASE IF EXISTS solemates;
...
DELIMITER $$
CREATE TRIGGER `product_sold_update` AFTER INSERT ON `purchase_item` FOR EACH ROW BEGIN
UPDATE inventory
...
DELIMITER $$
CREATE TRIGGER `new_employee_log` AFTER INSERT ON `employee` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
...
DELIMITER $$
CREATE TRIGGER `new_customer_log` AFTER INSERT ON `customer` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
...
DELIMITER $$
CREATE TRIGGER `new_brand_log` AFTER INSERT ON `brand` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
...
DELIMITER $$
CREATE TRIGGER `new_product_log` AFTER INSERT ON `product` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
...
DELIMITER $$
CREATE TRIGGER `new_rating_log` AFTER INSERT ON `rating` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
VALUES ('New rating added.');
END$$
DELIMITER ;
...
DELIMITER $$
CREATE TRIGGER `new_store_log` AFTER INSERT ON `store` FOR EACH ROW BEGIN
INSERT INTO log_event (event_description)
...
#F)
#1.
SELECT CONCAT(c.first_name, ' ', c.last_name) customer_name, p.product_name, pr.purchase_date
FROM customer c
JOIN purchase pr ON c.customer_id = pr.customer_id
JOIN purchase_item pi ON pr.purchase_id = pi.purchase_id
JOIN product p ON pi.product_id = p.product_id
WHERE pr.purchase_date BETWEEN '2021-05-01' AND '2022-05-01';

#2.
SELECT CONCAT(c.first_name, ' ', c.last_name) customer_name, SUM(p.product_price * pi.quantity) total_purchase
FROM customer c
JOIN purchase pr ON c.customer_id = pr.customer_id
JOIN purchase_item pi ON pr.purchase_id = pi.purchase_id
JOIN product p ON pi.product_id = p.product_id
...
#3.
SELECT
CONCAT(MIN(DATE_FORMAT(pr.purchase_date, '%m/%Y')), ' - ', MAX(DATE_FORMAT(pr.purchase_date, '%m/%Y'))) PeriodOfSales,
SUM(p.product_price * pi.quantity) TotalSales,
ROUND(SUM(p.product_price * pi.quantity) / (TIMESTAMPDIFF(YEAR, MIN(purchase_date), MAX(purchase_date)) + 1), 2) YearlyAverage,
ROUND(SUM(p.product_price * pi.quantity) / (TIMESTAMPDIFF(MONTH, MIN(purchase_date), MAX(purchase_date)) + 1), 2) MonthlyAverage
FROM purchase pr
JOIN purchase_item pi ON pr.purchase_id = pi.purchase_id
JOIN product p ON pi.product_id = p.product_id
WHERE pr.purchase_date BETWEEN '2021-01-01' AND '2023-01-01';

#4.
SELECT s.store_name location, SUM(p.product_price) total_sales
FROM store s
JOIN purchase pr ON s.store_id = pr.store_id
JOIN purchase_item pi ON pr.purchase_id = pi.purchase_id
JOIN product p ON pi.product_id = p.product_id
GROUP BY location;

#5.
SELECT DISTINCT s.store_name stores_with_reviews
FROM store s
JOIN purchase pr ON s.store_id = pr.store_id
JOIN purchase_item pi ON pr.purchase_id = pi.purchase_id
JOIN product p ON pi.product_id = p.product_id
WHERE p.product_id IN (SELECT DISTINCT r.product_id FROM rating r);
...
pr.purchase_date,
s.store_name,
SUM(pi.quantity * p.product_price) total_amount
FROM customer c
JOIN purchase pr ON c.customer_id = pr.customer_id
...
c.phone_number customer_phone_number,
p.product_id,
p.product_name,
p.product_price,
pi.quantity,
pi.quantity * p.product_price line_total
FROM purchase_item pi
JOIN purchase pr ON pi.purchase_id = pr.purchase_id
